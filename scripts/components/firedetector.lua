-- 不灭营火的标签列表
local NOTAGS = { "FX", "NOCLICK", "DECOR", "INLIMBO", "burnt", "player","monster","coldfire","firepit","campfire","coldfirepit"}
...
local function LookForFiresAndFirestarters(inst, self, force)
    ...
    -- 根据配置决定是否屏蔽营火
    local filterTags
    -- 修改点：不再比较 == 2，而是直接使用布尔值判断
    if TUNING.FS_PLUS_PROTECT_CAMPFIRE then 
        filterTags = { "FX", "NOCLICK", "DECOR", "INLIMBO", "burnt", "player","monster","coldfire","firepit","campfire","coldfirepit"}
    else 
        filterTags = { "FX", "NOCLICK", "DECOR", "INLIMBO", "burnt", "player","monster"}
    end

    local ents = TheSim:FindEntities(x, y, z, self.range, nil, filterTags, NONEMERGENCYTAGS)
    local target = nil
    local targetscore = 0
...
local function DetectFireEmergency(inst, self)
    ...
    local ents = TheSim:FindEntities(x, y, z, self.range, { "fire" }, filterTags)
...
local function OnDetectEmergencyTargets(inst, self)
    ...
    local ents = TheSim:FindEntities(x, y, z, self.range, nil, filterTags, EMERGENCYTAGS)
